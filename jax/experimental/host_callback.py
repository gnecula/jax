...
"""Primitives for calling user-defined Python functions on the host.
...
invoking a specified Python function (for :func:`id_tap`) or printing the
arguments (for :func:`id_print`). A few examples::
...
result is used. **At least one of the returned values must be used in the rest
of the computation, or else this operation has no effect.**

The data from the devices is received by separate threads managed by the JAX
runtime. The runtime maintains a buffer of configurable size. When the buffer
is full, the receiving threads are paused which eventually pauses the
computation on devices. The runtime has one additional thread that invokes the
Python user functions with the received data. If the processing of the callbacks
is slow, it may actually lead to the runtime buffer filling up, and eventually
pausing the computation on the devices when they need to send something.
For more details on the runtime mechanism see
`runtime code
<https://github.com/tensorflow/tensorflow/blob/master/tensorflow/compiler/xla/python/outfeed_receiver.cc>`_.

Exceptions from the user-defined tap functions are logged along with the
...
and it is practically impossible to use it under conditionals. Furthermore,
code that uses the outfeed mechanism directly cannot be transformed by JAX.

The tapping API introduced here makes it easy to share the outfeed mechanism
for multiple purposes, while supporting all transformations.
...
def _initialize_outfeed_receiver(
  ...
  with _outfeed_receiver.lock:
    ...
    if clients is None:
      ...
      clients = [c for c in clients if c.platform != "interpreter"]
    devices = list(itertools.chain(*[backend.devices() for backend in clients]))
    _outfeed_receiver.clients = clients
    _outfeed_receiver.devices = devices
    logging.vlog(
