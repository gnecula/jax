...
class PrintTokenState(threading.local):
  """Keeps the last computation and the last token."""
  # TODO: integrate this into the xla.py properly
  last_computation: Optional[XlaComputationBuilder]
  last_token: Optional[XlaOp]

  def __init__(self) -> None:
    self.last_computation = None
    self.last_token = None

  def get_last_token(self, comp: XlaComputationBuilder):
    if comp is not self.last_computation:
      self.last_computation = comp
      self.last_token = comp.CreateToken()
    return self.last_token

  def set_last_token(self, comp: XlaComputationBuilder, token: XlaOp):
    assert comp == self.last_computation
    self.last_token = token

print_token_state = PrintTokenState()
...
def _id_print_translation_rule_outfeed(
  ...
  def _outfeed_one_array(a: XlaOp, token: XlaOp) -> XlaOp:
    ...
    token = comp.Outfeed(comp.Constant(onp.array(descriptor, dtype=onp.int32)),
                                       token)
    token = comp.Outfeed(a, token)
    return token

  last_token = print_token_state.get_last_token(comp)
  nr_args_to_emit = len(args_op) - params.get("nr_results", 0)
  for i, a_op in enumerate(args_op):
    if i < nr_args_to_emit:
      last_token = _outfeed_one_array(a_op, last_token)
  print_token_state.set_last_token(comp, last_token)
  return comp.Tuple(*args_op)
